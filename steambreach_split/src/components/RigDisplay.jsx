...
function Trace({ points, color, active }) {
  ...
  return (
    <>
      <path d={d} fill="none" stroke={color} strokeOpacity="0.12" strokeWidth="4" />
      <path
        d={d}
        fill="none"
        stroke={color}
...
function MiniBar({ x, y, w, value, color }) {
  return (
    <>
      ...
      <rect x={x} y={y} width={Math.max(3, (w * value) / 100)} height="5" rx="999" fill={color} />
    </>
  );
}
...
export default function RigDisplay({
  ...
  const installed = useMemo(
    () => ({
      CPU: isInstalled(inventory, 'CPU'),
      GPU: isInstalled(inventory, 'GPU'),
      RAM: isInstalled(inventory, 'RAM'),
      SSD: isInstalled(inventory, 'SSD'),
      PSU: isInstalled(inventory, 'PSU'),
    }),
    [inventory]
  );
  ...
  const pwrPct = installed.PSU ? clamp(Math.round((cpuPct + gpuPct) / 2), 8, 100) : 0;
      ...
      {expanded ? (
        <svg width="500" height="250" viewBox="0 0 500 250">
          ...
          <g transform="translate(332 32)">
            ...
            <rect x="67" y="84" width="16" height="8" rx="1.5" fill={installed[selected] ? (selected === 'GPU' ? COLORS.proxy : COLORS.primary) : 'rgba(255,255,255,0.18)'} />
